...
app.get('/predict', async (req, res) => {
    ...
    res.json({
        "👑 id": "@ghetvietcode",
        "⚜️ Phiên Trước": currentPhien || "Đang tải...",
        "🎲 Xúc Xắc": `${latestOriginal?.facesList?.[0] || 'N/A'} - ${latestOriginal?.facesList?.[1] || 'N/A'} - ${latestOriginal?.facesList?.[2] || 'N/A'}`,
        "📈 Tổng Điểm": latestOriginal?.score || 0,
        "📊 Kết Quả": getResultType(latestOriginal) || "Chờ kết quả",
        "🎯 Phiên Dự Đoán": nextPhien,
        "🔮 Lựa Chọn AI": lastPrediction.du_doan,
        "🔢 Gợi Ý Vị": lastPrediction.doan_vi.join(" | "),
        "💎 Độ Tin Cậy": lastPrediction.do_tin_cay,
    });
